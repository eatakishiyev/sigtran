...
import azrc.az.sigtran.map.dialoguePDU.*;
import azrc.az.sigtran.map.parameters.*;
import azrc.az.sigtran.map.services.common.Result;
import azrc.az.sigtran.map.services.common.*;
import azrc.az.sigtran.map.services.errors.IncorrectErrorCodeException;
...
public abstract class MAPDialogue {
    ...
    private final Logger logger = LoggerFactory.getLogger(MAPDialogue.class);
        ...
        try {
            ...
            switch (mapDialogueState) {
                case DIALOGUE_INITIATED:
                    ...
                    //I'm closing MAP-Dialogue here, because in all diagrams(SMS/LU...)
...
//    }
    ...
    protected void onTCUError(TCUError indication) {
        ...
        if (requestingMapSSM == null) {
            ...
            } catch (IncorrectErrorCodeException | IncorrectSyntaxException | NoServiceParameterAvailableException |
                     UnexpectedDataException ex) {
                logger.error("ErrorOccured:", ex);
