...
import azrc.az.state.machines.State;
import azrc.az.state.machines.StateHandler;
...
public class AspStateMachine extends FiniteStateMachine {
    ...
    private void initFSM() throws Exception {
        if (asp.getNodeType() == NodeType.ASP || asp.getNodeType() == NodeType.IPSP_CLIENT) {
             ...
             */
            State stateASP_DOWN = createState(AspState.ASP_DOWN.name()).setEntryAction(() -> {

                boolean hasActiveAsp = false;
                List<Asp> aspList = as.getAspList();
                for (Asp otherAsp : aspList) {//Traverse through Application Servers Asp list to check if any active state Asp available
