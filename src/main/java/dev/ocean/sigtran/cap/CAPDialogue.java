...
import dev.ocean.sigtran.cap.errors.CAPErrorFactory;
import dev.ocean.sigtran.cap.errors.CAPUserError;
import dev.ocean.sigtran.cap.errors.SystemFailure;
import dev.ocean.sigtran.cap.errors.UnknownCapErrorException;
import dev.ocean.sigtran.cap.parameters.CapGPRSReferenceNumber;
import dev.ocean.sigtran.cap.parameters.CapUAbortReason;
import dev.ocean.sigtran.common.exceptions.*;
...
public abstract class CAPDialogue<T> {
    ...
    private final Logger logger = LogManager.getLogger(CAPDialogue.class);
...
//                break;
    ...
    private boolean isACNameUnchanged(ApplicationContextImpl applicationContextName) {
        ...
        if (problem.isInvokeProblem()) {
            ...
            if (requestingSSM == null) {
                ...
                this.onNegativeResultReceived(OperationCodes.getInstance(requestingSSM.getOperationCode()),
                        dev.ocean.sigtran.cap.ProviderError.SERVICE_COMPLETION_FAILURE, indication.getInvokeId());
            }
        } else {
            stack.fireNotice(Notifications.ABNORMAL_EVENT_RECEIVED_FROM_PEER, this);
        }
