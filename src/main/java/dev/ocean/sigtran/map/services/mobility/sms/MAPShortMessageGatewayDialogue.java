...
import dev.ocean.sigtran.map.parameters.LocationInfoWithLMSI;
...
import dev.ocean.sigtran.map.parameters.MWStatus;
import dev.ocean.sigtran.map.parameters.SMDeliveryOutcome;
...
public class MAPShortMessageGatewayDialogue extends MAPDialogue {
    ...
    private final static Logger logger = LogManager.getLogger(MAPShortMessageGatewayDialogue.class);
    ...
    private void processSriForSMConfirmation(TCResult indication) {
        ...
        } catch (UnexpectedDataException | IncorrectSyntaxException ex) {
