...
import dev.ocean.sigtran.m3ua.parameters.ConcernedDPC;
import dev.ocean.sigtran.m3ua.parameters.CongestionIndication;
 ...
 * @author root
 */
public class SignallingCongestion extends Message {

    private final MessageClass messageClass = MessageClass.SSNM;
    private final MessageType messageType = MessageType.SCON;
    private NetworkAppearance networkAppearance;
    private RoutingContext routingContext;
    private AffectedPointCode affectedPointCode;
    private ConcernedDPC concernedDPC;
    private CongestionIndication congestionIndication;
    private InfoString infoString;

    @Override
    public String toString() {
        return String.format("SignallingCongestion [NetworkAppearance = %s; RoutingContext = %s;"
                + "AffectedPointCode = %s; ConcernedDpc = %s; CongestionIndication = %s; "
                + "InfoString = %s]", this.networkAppearance, this.routingContext,
                this.affectedPointCode, this.concernedDPC, this.congestionIndication, this.infoString);
    }
    ...
    public void encode(M3UAMessageByteArrayOutputStream mbaos) throws IOException {
        ...
        if (concernedDPC != null) {
            concernedDPC.encode(tmpBaos);
        }
        if (congestionIndication != null) {
            congestionIndication.encode(tmpBaos);
        }
        if (infoString != null) {
        ...
        while (mbais.available() > 0) {
            ...
            switch (tag) {
                ...
                case CONCERNED_DESTINATIONS:
                    concernedDPC = new ConcernedDPC();
                    concernedDPC.decode(mbais);
                    break;
                case CONGESTION_INDICATIONS:
                    congestionIndication = new CongestionIndication();
                    congestionIndication.decode(mbais);
                    break;
                case INFO_STRING:
    ...
    /**
     * @param concernedDPC the concernedDPC to set
     */
    public void setConcernedDPC(ConcernedDPC concernedDPC) {
        this.concernedDPC = concernedDPC;
    }

    /**
     * @return the congestionLevel
     */
    public CongestionIndication getCongestionIndication() {
        return congestionIndication;
    }

    /**
     * @param congestionIndication
     */
    public void setCongestionLevel(CongestionIndication congestionIndication) {
        this.congestionIndication = congestionIndication;
    }
