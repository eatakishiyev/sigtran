...
import dev.ocean.sigtran.sccp.general.RemoteSignallingPoint;
import dev.ocean.sigtran.sccp.general.RemoteSubSystem;
import dev.ocean.sigtran.sccp.general.SCCPStackImpl;
import dev.ocean.sigtran.sccp.general.SubSystem;
...
import dev.ocean.sigtran.sccp.messages.connectionless.UnitDataService;
...
import dev.ocean.sigtran.sccp.messages.management.SCCPManagementMessageFactory;
import dev.ocean.sigtran.sccp.messages.management.SubsystemAllowed;
import dev.ocean.sigtran.sccp.messages.management.SubsystemProhibited;
import dev.ocean.sigtran.sccp.messages.management.SubsystemStatusTest;
...
//    private static transient Logger logger = SCCPStackImpl.logger;
    ...
    private final Logger logger = LogManager.getLogger(SCCPConnectionlessControlImpl.class);
    ...
    public void messageForLocalSubsystem(SCCPMessage message, boolean localOriginator) throws Exception {
        ...
        sccpStack.messageForLocalSSN.increment();
                ...
                if (message.getCalledPartyAddress().getSubSystemNumber() == SubSystemNumber.SCCP_MANAGEMENT) {
                    ...
                    if (segmentation != null) {//Segmentation parameter
                        ...
                        if (segmentation.getRemainingSegment() == 1) {//Single segment 
                            ...
                            if (reassembler == null) {//Idle
                                if (segmentation.getFirstSegmentIndication() == 1) {//First segment
                                    ...
                                    sccpStack.reassemblyErrorCounter.increment();
                        ...
                        if (message.getCalledPartyAddress().getSubSystemNumber() == SubSystemNumber.SCCP_MANAGEMENT) {
                            ...
                            sccpStack.unitDataCounter.increment();
                            subSystem.onMessage(message);
                        }
                    }
                }
    ...
    public void routingFailure(SCCPMessage message, ErrorReason sccpErrorReason,
        ...
        switch (message.getType()) {
            case UDT:
                ...
                if (localOriginator) {
                    SubSystemNumber ssn = unitData.getCalledPartyAddress().getSubSystemNumber();
...
//      concerned SP.   
    ...
    protected void processManagementMessage(SCCPMessage message) throws Exception {
        ...
        switch (managementMessage.getMessageType()) {
            case SSP:
                sccpStack.sspCounter.increment();
                ...
                break;
            case SSA:
                sccpStack.ssaCounter.increment();
                ...
                break;
            case SST:
                sccpStack.sstCounter.increment();
