...
//    private static transient Logger logger = SCCPStackImpl.logger;
    private final transient Logger logger = LogManager.getLogger(SCCPRoutingControlImpl.class);
    ...
    public void onMtpTransferIndication(MTPTransferMessage mtpTransferMessage) {
        ...
        sccpStack.packetsReceived.increment();
        ...
        try {
            if (message == null) {
                ...
                sccpStack.unknownMessageTypeCounter.increment();
                return;
            }
                ...
                case ROUTE_ON_GT:
                    if (message.getType() == MessageType.XUDT
                        ...
                        if (hopCounter <= 0) {
                            ...
                            sccpStack.hopCounterViolation.increment();
            ...
            case XUDT:
                if (isTranslated) {
                    ...
                    if (mtpSap == null) {
                        ...
                        sccpStack.mtpFailureCounter.increment();
                        return;
                    }
                    if (localOriginator) {
    ...
    private void processRouteOnSSN(SCCPMessage message, boolean localOriginator) throws Exception {
        if (!message.getCalledPartyAddress().getAddressIndicator().getSSNIndicator()) {
            sccpStack.unqualifiedErrorCounter.increment();
        ...
        if (ssn == SubSystemNumber.SCCP_MANAGEMENT) {
            ...
            if (subsystem == null) {
                sccpStack.unequippedUserCounter.increment();
            ...
            if (subsystem.getState() == SubSystem.State.IDLE) {
                sccpStack.subsystemFailureCounter.increment();
    ...
    private void processRouteOnGt(SCCPMessage message, boolean localOriginator) throws Exception {
        ...
        if (globalTitleTranslator == null) {
            sccpStack.gttFailureAddressNature.increment();
        ...
        if (rule == null) {
            sccpStack.gttFailureSpecAddress.increment();
        ...
        if (entitySet == null) {
            sccpStack.sccpFailureCounter.increment();
